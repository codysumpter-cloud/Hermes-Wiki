...
TOOLSETS = {
    ...
    # X (Twitter) 搜索工具集（xAI 凭证可用时启用）
    "x_search": {
        "description": "x_search Responses tool",
        "tools": ["x_search"],
        "includes": []
    },
...
| `hermes-wecom-callback` | 企业微信回调 | 企业微信回调通道 |
| `hermes-webhook` | Webhook | 接收外部事件 |
| `hermes-acp` | 编辑器集成 | 编码专用 |
| `hermes-api-server` | HTTP API | 通过 HTTP 访问 |
| `hermes-cron` | Cron 调度 | 定时任务触发 |
| `hermes-bluebubbles` | BlueBubbles (iMessage) | iMessage 桥接 |
| `hermes-weixin` | 微信 | 微信消息 |
| `hermes-qqbot` | QQ 机器人 | QQ 消息 |
| `hermes-yuanbao` | 腾讯元宝 | 元宝集成 |
| `hermes-gateway` | Gateway | 网关运行时 |

> 注：`toolsets.py` 共定义 24 个 `hermes-*` 前缀的工具集（上表全部 24 行）。其中 `hermes-cli` / `hermes-acp` / `hermes-api-server` / `hermes-gateway` 属于运行时入口工具集，其余为消息平台工具集。
