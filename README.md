...
- [kanban-multi-profile-board](concepts/kanban-multi-profile-board.md): SQLite-backed 跨 profile 协作板（v0.12.0），WAL+CAS、15 verbs CLI、worker tools、dashboard 插件
- [persistent-goals-ralph-loop](concepts/persistent-goals-ralph-loop.md): `/goal` Ralph 循环（v0.12.0），辅助模型 judge + 不动系统提示 + SessionDB 持久化
